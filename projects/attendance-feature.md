...
- 퇴근(clock-out)에도 출근과 동일하게 GPS 영역 검증 적용 (2026-03-04 변경)

---

## 8. Kakao Maps 출퇴근 지도 다이얼로그 (2026-03-04 추가)

### 8.1 개요

출근/퇴근 버튼 클릭 시 Kakao Maps 지도 다이얼로그를 표시하여, 현재 위치와 등록된 근태 영역을 시각적으로 보여주는 기능. 출퇴근 가능 여부를 지도 위에서 직관적으로 확인할 수 있다.
...
출근하기/퇴근하기 클릭
  → GPS 위치 + 활성 영역 동시 요청
  → 영역 0개: 바로 출근/퇴근 처리 (기존 동작)
  → 영역 1개 이상: 지도 다이얼로그 표시 (mode='clock-in' 또는 'clock-out')
    → 반경 내: 초록색 "출근/퇴근 가능 지역입니다" + 확인 버튼 활성
    → 반경 밖: 빨간색 "현재 출근/퇴근 가능 지역이 아닙니다" + 가장 가까운 영역까지 거리 표시 + 버튼 비활성
```
...
- **Kakao Maps SDK 동적 로딩**: `<script>` 태그로 `https://dapi.kakao.com/v2/maps/sdk.js` 로드 (10초 timeout)
- **환경변수**: `VITE_KAKAO_MAP_KEY` — Kakao Developers JavaScript 키
- **Haversine 거리 계산**: 프론트엔드에서도 거리를 계산하여 UI 표시
- **지도 요소**:
  - 현재 위치: 빨간색 마커 + 반투명 파란색 정확도 원
  - 근태 영역: Circle (반경 시각화, 초록색) + CustomOverlay (영역 이름)
- **Props**: `open`, `onOpenChange`, `latitude`, `longitude`, `zones`, `onConfirm`, `loading`, `mode` ('clock-in' | 'clock-out')
- **mode에 따른 UI 분기**: 제목, 상태 메시지, 버튼 텍스트/아이콘이 출근/퇴근에 맞게 변경
- **모바일 반응형**: 지도 높이 200px(모바일)/300px(데스크톱), 버튼 풀와이드(모바일)

#### AttendancePage.jsx ClockTab 변경사항

- `handleClockInClick`: GPS + `getActiveZones()` 동시 호출 → 영역 유무에 따라 분기
- `handleClockOutClick`: 퇴근도 출근과 동일한 플로우 (GPS + 영역 조회 → 다이얼로그)
- `handleConfirmFromMap`: 지도 다이얼로그에서 확인 클릭 → mode에 따라 출근/퇴근 API 호출
...
## 9. 모바일 반응형 최적화 (2026-03-04 추가)

### 9.1 ClockTab (출퇴근)

| 요소 | 데스크톱 (≥640px) | 모바일 (<640px) |
|------|-----------------|----------------|
| 시계 | text-5xl | text-3xl |
| 출근/퇴근 카드 | 2열 (grid-cols-2) | 1열 (grid-cols-1) |

### 9.2 RegisterTab (근태 등록)

- select box에 `appearance-none` + 커스텀 chevron SVG 적용 (화살표 위치 개선)

### 9.3 RecordsTab (내 근태 현황)

| 요소 | 데스크톱 (≥640px) | 모바일 (<640px) |
|------|-----------------|----------------|
| 날짜 필터 | 가로 1행 (flex-row) | 날짜 2열 + 조회 버튼 별도 행 |
| 통계 카드 5개 | 5열 (grid-cols-5) | 3열 (grid-cols-3, 3+2 줄바꿈) |
| 근태 기록 | 테이블 (6컬럼) | 카드 리스트 (날짜+상태+출퇴근시간) |

### 9.4 ClockInMapDialog

| 요소 | 데스크톱 (≥640px) | 모바일 (<640px) |
|------|-----------------|----------------|
| 지도 높이 | 300px | 200px |
| 다이얼로그 너비 | max-w-lg | calc(100%-2rem) |
| 버튼 | 인라인 | 풀와이드, 세로 배치 |
...
- [x] GPS 출퇴근 모바일 반응형 최적화 (9절)
- [x] 퇴근에도 GPS 영역 체크 + 지도 다이얼로그 적용 (8절)
