...
│   ├── attendanceService.ts    # 근태 관리 비즈니스 로직
...
│   ├── attendanceRepository.ts       # 근태 관리 DB 접근
│   └── organizationRepository.ts     # 조직 관리 DB 접근
├── types/
│   ├── attendance.ts      # 근태 관리 타입 정의
...
2026-03-03 구현. GPS 기반 출퇴근 + 근태 등록 + HR 관리자 조회/통계.

### API 엔드포인트
- `GET /api/attendance/types` — 근태 유형 목록
- `GET /api/attendance/today` — 오늘 내 출근 기록
- `POST /api/attendance` — 출퇴근/근태 등록 (action: clock-in / clock-out / register)
- `GET /api/attendance` — 내 근태 기간 조회
- `GET /api/attendance/admin` — HR 관리자 전 직원 근태 조회 (hr/superadmin 역할 필수)
- `GET /api/attendance/stats` — 근태 통계 (hr/superadmin 역할 필수)

### 허용 역할
- 일반: 인증된 모든 사용자 (types, today, clock-in/out, register, 내 기록)
- HR 관리자: `hr`, `superadmin` (admin, stats)

### DB 스키마
- 테이블: `organization.attendance_types`, `organization.attendance_records`
- 상세: [db-schemas/attendance-schema.md](../db-schemas/attendance-schema.md)
- API 상세: [api-specs/attendance-api.md](../api-specs/attendance-api.md)
