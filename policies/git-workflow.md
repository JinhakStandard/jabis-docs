...
main에서 분기 → feature 브랜치 작업 → commit → push
    → Bitbucket PR 생성 (→ main) → navskh 승인
    → Merge → main Pipeline 자동 배포
```

**Alpha 경유 배포 (권장):**
```
main에서 분기 → feature 브랜치 작업 → commit
    → feature를 alpha에 머지 → alpha push → Alpha 환경 배포 & 테스트
    → alpha → main PR 생성 → navskh 승인
    → Merge → main Pipeline 운영 배포
```

> alpha 경유 플로우의 상세 절차는 "4. Alpha 브랜치 전략" 참조.
...
Alpha 환경은 운영 배포 전 검증 단계입니다. feature 브랜치에서 alpha로, alpha에서 main으로 단계적으로 머지합니다.

```
feature/* ──(push)──→ alpha ──(Pipeline)──→ Alpha 환경 배포 & 테스트
                         │
                         └──(PR/merge)──→ main ──(Pipeline)──→ 운영 배포
```
...
3. feature → alpha 머지 (alpha 환경 배포)
...
4. alpha 환경에서 테스트/검증
   → *-alpha.jinhakapply.com 접속하여 기능 확인

5. alpha → main 머지 (운영 배포)
   → Bitbucket PR 생성 (source: alpha → destination: main)
   → navskh 승인 → Merge
   → main Pipeline → 운영 환경 자동 배포
```
...
| 항목 | 규칙 |
|------|------|
| alpha push | feature 머지 후 push — Pipeline 자동 트리거 |
| alpha → main | **PR 필수** (navskh 승인) |
| alpha 직접 커밋 | 가능 (긴급 alpha 테스트용) |
| alpha 리셋 | 주기적으로 main에서 리셋 가능 (`git reset --hard main`) |

### 4.3 jabis-common submodule 동기화

jabis-common은 alpha 브랜치 없이 **master만 사용**합니다. Pipeline이 없는 순수 소스 저장소이므로 브랜치 분리가 불필요합니다.
...
[2] 소비 프로젝트 alpha 브랜치에서:
    ...
    git push origin alpha
    → alpha Pipeline → alpha 환경 배포 → 테스트

[3] alpha 검증 통과 후:
    alpha → main PR 생성 → navskh 승인 → Merge
    → main Pipeline → 운영 배포
```

**핵심:**
- submodule 참조 커밋이 잠금장치 역할 — main 브랜치는 머지 전까지 이전 커밋 참조 유지
- alpha → main 머지 시 submodule 참조 커밋도 함께 머지됨
- jabis-common 자체는 master만 사용하므로, 소비 프로젝트의 alpha/main 브랜치가 어떤 커밋을 참조하느냐로 버전 관리

> **상세 정책**: `policies/staging.md` 5절 참조.

---

## 5. Claude Code PR 자동 생성
...
curl -X POST ... (위 5.2 참조)

# 6. 사용자에게 PR URL 안내
```

### 5.4 submodule 포함 작업 시 PR 순서
...
## 9. Bitbucket Branch Permission 설정 절차

전체 JABIS 저장소에 main 브랜치 보호를 설정하는 절차입니다.

### 9.1 저장소별 설정 (Bitbucket 웹)
