...
### Alpha-only push 정책 (필수)

**alpha 브랜치가 있는 프로젝트는 반드시 alpha 브랜치에만 push한다. master/main 직접 push 금지.**

대상 프로젝트: jabis, jabis-api-gateway, jabis-hr, jabis-dev, jabis-producer, jabis-finance, jabis-sysadmin, jabis-storage

**AI 작업 시 규칙:**
1. 위 프로젝트 작업 시 반드시 `alpha` 브랜치인지 확인 후 작업 시작 (`git branch --show-current`)
2. alpha가 아니면 `git checkout alpha && git pull origin alpha`로 전환
3. push 대상은 항상 `git push origin alpha`
4. master/main에 직접 push하는 코드를 절대 실행하지 않음
5. 운영 반영이 필요하면 Bitbucket에서 alpha → master PR 생성 후 머지

**pre-push hook (로컬 안전장치):**
- 공유 hook: `/home/jinhak/Projects/.git-hooks/pre-push`
- 위 8개 프로젝트의 `.git/hooks/pre-push`에 심볼릭 링크로 설치
- master/main push 시도 시 자동 차단
- 우회 필요 시: `ALLOW_MAIN_PUSH=1 git push origin master`
