...
Alpha(스테이징) 환경은 운영 배포 전 검증 목적으로 사용합니다.

```
alpha 브랜치 push → Bitbucket Pipeline 트리거
→ Docker Build → Harbor Push (jabis-alpha 네임스페이스)
→ jabis-helm alpha values 업데이트 → Helm Package → Harbor Push
→ ArgoCD 자동 감지 → K3S jabis-alpha 네임스페이스 배포
→ Teams 알림
```
...
| 인프라 | 구성 | 상세 |
|--------|------|------|
| K3S 네임스페이스 | `jabis-alpha` | alpha 전용 |
| PostgreSQL | **운영 공유** | 같은 서버, `jabis_alpha` DB 별도 |
| Redis | **운영 공유** | 같은 인스턴스, DB 1 (운영 DB 0) |
| jabis-cert | **운영 공유** | alpha 전용 client_id 별도 발급 |
| MinIO | **운영 공유** | 같은 버킷, `alpha/` prefix로 경로 분리 |
| Harbor | `jabis-alpha` 프로젝트 | alpha 이미지 저장소 |
| 도메인 | `*-alpha.jinhakapply.com` | 와일드카드 DNS |
...
**Helm 버전 충돌 방지를 위한 Retry 메커니즘이 적용되어 있다.**

각 파이프라인의 `update-helm` 단계는 jabis-helm 리포지토리를 clone하여 Chart.yaml 버전을 증가시키고 values 파일의 이미지 태그를 업데이트한다. 동시에 여러 파이프라인이 실행될 경우 충돌이 발생할 수 있으므로, **fetch → reset → 재계산 → push (non-force) → 실패 시 retry** 루프로 처리한다.

**동작 방식:**
1. 매 시도마다 현재 브랜치를 감지하여 `git fetch origin $BRANCH && git reset --hard origin/$BRANCH`로 최신 상태 동기화
2. 최신 Chart.yaml에서 버전을 읽어 PATCH +1 증가
3. `git push` (non-force)로 커밋 — 충돌 시 자동 retry (최대 5회, 3~8초 랜덤 대기)

**규칙:**
1. **동시 push 가능** — retry 메커니즘이 충돌을 자동 해결하므로 간격 없이 push해도 안전하다
2. 의존 관계가 있으면 하위 서비스부터 배포한다 (예: gateway → frontend)
3. feature 브랜치 push는 파이프라인이 트리거되지 않으므로 간격 불필요

```bash
# 배포 예시 (main 브랜치) — 간격 없이 연속 push 가능
cd jabis-api-gateway && git push
cd ../jabis-hr && git push
cd ../jabis && git push
```

> **참고:** 이전에는 30초 간격을 권장했으나, retry 메커니즘(최대 5회, 3~8초 랜덤 대기)이 충분히 안정적이므로 간격 없이 push해도 문제없다.
...
### 원칙
- **DDL/마이그레이션 쿼리는 사용자가 직접 실행한다** — AI가 파일로만 저장하고 끝내면 안 됨
- AI가 스키마를 변경하는 작업을 수행할 때, **반드시 실행할 SQL을 채팅에 직접 출력**하여 사용자에게 전달해야 한다
- 쿼리 파일 저장은 기록용으로 하되, 그것만으로 완료 처리하지 않는다

### AI 작업 시 필수 플로우

```
1. 스키마 변경이 필요한 작업 수행
2. 마이그레이션 SQL 파일 생성 (sql/migrate-xxx.sql)
3. ⚠️ 채팅에 실행할 쿼리 전문을 출력하여 사용자에게 전달
4. 사용자가 쿼리 실행 완료 확인 후 → 코드 배포 (push)
```

> **배포 순서**: DB 마이그레이션 쿼리 실행 → 코드 push (배포)
> 코드가 먼저 배포되면 새 스키마를 참조하는 쿼리가 실패한다.

### 마이그레이션 SQL 파일 관리
- 위치: `jabis-api-gateway/sql/migrate-{설명}.sql`
- 반드시 `BEGIN...COMMIT` 트랜잭션으로 감쌈
- 백업 테이블 생성 포함 (확인 후 수동 삭제)
...
### URL 형식
```
http://{서비스명}-{stage}-service.{네임스페이스}-{stage}:{포트}
```
...
Alpha 환경은 `jabis-alpha` 네임스페이스를 사용합니다.

| 서비스 | 내부 URL | 외부 도메인 |
|--------|----------|-------------|
| jabis-api-gateway | `http://jabis-api-gateway-alpha-service.jabis-alpha:3100` | jabis-gateway-alpha.jinhakapply.com |
| jabis-storage | `http://jabis-storage-alpha-service.jabis-alpha:3400` | 없음 (gateway 프록시) |

> **jabis-cert는 alpha 별도 배포하지 않음** — 운영 jabis-cert를 공유하며, alpha 전용 client_id로 구분합니다.
> Alpha 서비스가 cert를 호출할 때는 **운영 내부 URL** (`http://jabis-cert-prod-service.jabis-prod:3000`)을 사용합니다.
