...
**규칙:**
1. **30초 간격은 권장사항** (retry가 있으므로 필수는 아니지만, 불필요한 retry를 줄이기 위해 권장)
...
sleep 30  # 권장 (retry 횟수 최소화)
cd ../jabis-hr && git push
sleep 30
cd ../jabis && git push
```

> **참고:** retry 메커니즘 덕분에 동시 push해도 데이터 손실은 발생하지 않지만, 30초 간격을 두면 파이프라인 실행 시간을 단축할 수 있다.
